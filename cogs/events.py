...
class URLCache:
    ...
    def add_result(self, url: str, result: dict, expire_time: str):
        ...
        # Parse the expire time from API response
        try:
            # Remove the 'Z' and parse the ISO format
    ...
    def get_result(self, url: str) -> dict:
        ...
        try:
            ...
            # Handle case where expire_time might be a string
            ...
            # Normalize expire_time to aware UTC if needed
    ...
    def cleanup_expired(self):
        ...
        for url, expire_time in self.expiry_times.items():
            try:
                ...
                # Normalize to aware UTC if needed
                if isinstance(expire_time, datetime) and expire_time.tzinfo is None:
                    expire_time = expire_time.replace(tzinfo=timezone.utc)
                    self.expiry_times[url] = expire_time

                # Now compare datetime objects (aware)
...
class Events(commands.Cog):
    def __init__(self, bot: commands.Bot):
        ...
        # Timeout functionality integrated directly
        
        # NSFW invite report cooldown to prevent spam
        self.nsfw_invite_report_cooldowns: Dict[int, datetime] = {}
        self.nsfw_invite_report_cooldown_duration = timedelta(minutes=5)  # 5 minute cooldown per user
        ...
        # Log what each keyword normalizes to
        for keyword in self.nsfw_keywords:
            normalized = self._normalize_text(keyword)
        
        # URL patterns for NSFW detection
        self.url_pattern = re.compile(r'https?://[^\s<>"\']+')

    def _normalize_text(self, text: str) -> str:
        """Normalize text to resist obfuscation (strip accents, lower, remove non-alphanumerics)."""
        try:
    ...
    def _check_nsfw_keyword_match(self, text: str, keyword: str) -> bool:
        ...
        # Special handling for age-related keywords to avoid false positives
        if keyword in ["18+", "+18"]:
            # For age keywords, require the full pattern, not just "18"
            return keyword.lower() in text.lower()
        
        return keyword_norm and keyword_norm in text_norm
    
    def _should_send_nsfw_invite_report(self, user_id: int) -> bool:
        """Check if we should send an NSFW invite report for this user (cooldown check)."""
        current_time = discord.utils.utcnow()
        
        # Check if user has a recent report
        ...
        # Update cooldown and allow report
    ...
    def _cleanup_expired_nsfw_invite_cooldowns(self):
        """Clean up expired NSFW invite report cooldowns to prevent memory bloat."""
        current_time = discord.utils.utcnow()
        expired_users = []
    ...
    async def scan_message_with_ai(self, message: discord.Message) -> bool:
        ...
        try:
            ...
            # Check if we should scan this message type
            ...
            if scan_result.get('should_flag', False):
                ...
                return True  # Don't delete message, let moderators decide
            
            return True
            
        except Exception as e:
            logger.error(f"Error scanning message with AI: {e}")
            return True  # Allow message if scanning fails
    ...
    async def handle_ai_moderation_report(self, message: discord.Message, scan_result: Dict[str, any]):
        ...
        try:
            # Check AI confidence level - skip low confidence reports
            ...
            # Skip reports with very low confidence or obvious content
            ...
            if report_message:
                # Send image attachments if any - always send media to AI reports channel
                ...
                logger.info(f"Sent AI moderation report for message {message.id} from {message.author.display_name} (confidence: {confidence:.3f})")
            else:
    ...
    async def _is_discord_invite_nsfw(self, url: str) -> bool:
        ...
        This method fetches invite information and checks:
        - Server name for NSFW keywords
        - Server description for NSFW content
        - Member count (very low counts might indicate fake servers)
        - URL path for NSFW keywords
        """
        try:
            ...
            try:
                ...
                # Check member count (suspicious if very low)
                if invite.approximate_member_count and invite.approximate_member_count < 5:
                    return True
                
                # Check URL path for NSFW keywords as fallback
                for keyword in self.nsfw_keywords:
                ...
                # If invite not found, check URL for keywords as fallback
                for keyword in self.nsfw_keywords:
                    if self._check_nsfw_keyword_match(url, keyword) or self._check_nsfw_keyword_match(path_lower, keyword):
                        return True
                return False
                ...
                # If access forbidden, check URL for keywords as fallback
                for keyword in self.nsfw_keywords:
                    if self._check_nsfw_keyword_match(url, keyword) or self._check_nsfw_keyword_match(path_lower, keyword):
                        return True
                return False
            ...
            # Fallback to URL keyword checking
            try:
                for keyword in self.nsfw_keywords:
                    if self._check_nsfw_keyword_match(url, keyword) or self._check_nsfw_keyword_match(path_lower, keyword):
                        return True
    ...
    async def check_nsfw_links(self, content: str) -> tuple[bool, list]:
        ...
        for url in urls:
            ...
            url = url.strip()
            if not url or len(url) < 10:  # Skip very short URLs
                continue
                
            # Remove trailing punctuation that might be part of the message
            url = url.rstrip('.,;:!?')
            ...
            # Skip API endpoints and internal URLs (but allow test URLs)
            if any(skip in url.lower() for skip in ['webrisk.googleapis.com', 'localhost', '127.0.0.1']):
                continue

            # Whitelist safe domains (discord and tenor), but still scan Discord invite links
            try:
                from urllib.parse import urlparse
                parsed = urlparse(url)
                ...
                # Domains to always allow (non-invite Discord resources and Tenor)
                safe_hosts = {
                    'discord.com',
                    'discordapp.com',
                    'cdn.discordapp.com',
                ...
                # Check Tenor (any subdomain of tenor.com)
                is_tenor = host == 'tenor.com' or host.endswith('.tenor.com')

                # Discord invites: skip WebRisk but still run internal checks
                is_discord_invite = (
                    host == 'discord.gg' or
                ...
                # If it's a whitelisted Discord host and not an invite, skip scanning
                is_safe_discord_resource = (host in safe_hosts) and not is_discord_invite

                # Handle Discord invites via internal checks only
                if is_discord_invite:
                    try:
                        ...
                        logger.error(f"Error checking Discord invite NSFW status: {e}")
                    # Always skip WebRisk for invites
                    continue
                    ...
                    async with session.get(
                        ...
                        if response.status == 200:
                            ...
                            if data.get("threat") and data["threat"].get("expireTime"):
                                ...
                                # Cache safe results for 1 hour (aware UTC)
    ...
    async def check_nsfw_content(self, content: str) -> tuple[bool, list]:
        ...
        if has_malicious_links:
            logger.info(f"Returning True due to malicious links")
            return True, malicious_urls
        
        logger.info(f"No NSFW content detected, returning False")
        return False, []
    ...
    async def handle_spam_detection(self, message: discord.Message):
        ...
        # Check for spam with more realistic burst thresholds
        # Allow 8 messages in 15 seconds (more lenient for natural bursts)
        recent_messages_15s = [msg for msg in self.message_history[user_id] 
                              if current_time - msg < timedelta(seconds=15)]
        
        # Also check for extreme spam (12+ messages in 10 seconds)
        recent_messages_10s = [msg for msg in self.message_history[user_id] 
                              if current_time - msg < timedelta(seconds=10)]
        
        # Trigger on moderate spam (8+ in 15s) or extreme spam (12+ in 10s)
        if len(recent_messages_15s) >= 8 or len(recent_messages_10s) >= 12:
            ...
            if self.spam_warnings[user_id] >= 3:
                ...
                try:
                    ...
                    await message.channel.send(f"⏰ **{message.author.display_name}** has been timed out for 5 minutes due to spam.")
    ...
    # Timeout implementation integrated directly

    @commands.Cog.listener()
    async def on_message(self, message: discord.Message):
        """Handle message events for spam and NSFW detection."""
        if message.author.bot or not message.guild:
            # Check for suspicious DM activity if it's a DM
        ...
        # AI Moderation scanning
        await self.scan_message_with_ai(message)
            
        # Check for Discord invite links to banned servers
        ...
        if is_nsfw:
            ...
            try:
                # Remove the NSFW message
                await message.delete()
                await message.channel.send(f"**{message.author.display_name}**, NSFW or Malicious content is not allowed.", delete_after=10)
                ...
                try:
                    embed = EmbedDesign.warning(
                        ...
                        description=f"Your message in **{message.guild.name}** was automatically removed due to containing NSFW or malicious content.\n\n**Server:** {message.guild.name}\n**Channel:** #{message.channel.name}\n\nThis is an automated action to protect server members."
                    )
                    if malicious_urls:
                        embed.add_field(name="Detected URLs", value=f"• {chr(10).join(malicious_urls[:3])}" + ("..." if len(malicious_urls) > 3 else ""), inline=False)
                ...
                # Always send report to main server regardless of user role
                ...
                # Only timeout users that are not developers/staff (cross-guild aware)
                ...
                if not (StaffUtils.is_staff(message.author) or is_developer):
                    ...
                    try:
                        # Timeout user for 24 hours for NSFW content
                        ...
                        await message.channel.send(f"**{message.author.display_name}** has been timed out for 24 hours due to NSFW content.")
                        logger.info(f"User {message.author.display_name} timed out for NSFW content")
                    except Exception as e:
                        logger.error(f"Failed to timeout user {message.author.display_name}: {e}")
                        # Don't show timeout message if it failed
                else:
    ...
    async def on_interaction(self, interaction: discord.Interaction):
        ...
        # Check if this is a moderation report button
    ...
    async def _handle_moderation_button(self, interaction: discord.Interaction, custom_id: str):
        ...
        try:
            # Parse the custom ID to get action and report ID
            parts = custom_id.split('_', 2)
            if len(parts) != 3:
                return
            
            action = parts[1]  # accept, deny, or view
            report_id = parts[2]  # guild_channel_message format
    ...
    async def _handle_accept_report(self, interaction: discord.Interaction, report_id: str):
        ...
        try:
            # Parse report ID to get message info
    ...
    async def _handle_view_message(self, interaction: discord.Interaction, report_id: str):
        ...
        # (Discord invite NSFW check logic removed from here; it should be handled in on_message or a dedicated method where 'message' is defined.)
    ...
    async def send_nsfw_report(self, message: discord.Message, malicious_urls: list = None):
        ...
        try:
            # Get the main server and report channel
            main_server_id = constants.main_server_id()
            report_channel_id = constants.report_channel_id()
            
            logger.info(f"Attempting to send NSFW report - Main Server ID: {main_server_id}, Report Channel ID: {report_channel_id}")
            ...
            # Determine action taken based on user role (cross-guild aware)
            is_staff = StaffUtils.is_staff(message.author) or await StaffUtils.has_developer_permission_cross_guild(self.bot, message.author)
            ...
            if malicious_urls:
                fields.append({"name": "Malicious URLs", "value": "\n".join([f"• {url}" for url in malicious_urls]), "inline": False})
    ...
    async def on_member_join(self, member: discord.Member):
        ...
        # Suspicious activity detection for new joins
        detector = get_suspicious_activity_detector(self.bot)
        ...
        if blacklist_record:
            # Don't auto-ban on dev server (appeal server)
    ...
    async def send_staff_log(self, guild: discord.Guild, embed: discord.Embed):
        ...
        try:
            # Get server's log configuration
            ...
            if log_config:
                ...
                # Fallback to staff channels if no log config
    ...
    async def log_uec_ban(self, member: discord.Member, reason: str, blacklisted_by: str, ban_successful: bool = True):
        ...
        # Override the color for failed bans
    ...
    async def check_alt_evasion(self, member: discord.Member):
        ...
        try:
            # Get Roblox ID for the joining user
            roblox_id = None
            try:
                async with aiohttp.ClientSession() as session:
            ...
            # Check if this Roblox ID is associated with any banned user
            # Note: This query needs to be handled differently in PostgreSQL
            # For now, we'll skip this check as it requires schema changes
            banned_record = None
            ...
            if banned_record:
                ...
                # Get the main banned user's ID for the ban reason
                ...
                # Log the alt evasion ban attempt, regardless of ban success
                await self.log_alt_evasion_ban(member, roblox_id, main_user_id, main_reason, blacklisted_by, ban_successful)
    ...
    async def log_alt_evasion_ban(self, member: discord.Member, roblox_id: str, main_user_id: int, main_reason: str, blacklisted_by: str, ban_successful: bool = True):
    ...
    async def handle_raid_detection(self, member: discord.Member):
        ...
        # Check for raid (more than 20 joins in 30 seconds)
        recent_joins = [join_time for join_time in self.recent_joins[guild_id]
                       if current_time - join_time < timedelta(seconds=30)]
        
        if len(recent_joins) >= 20:
            # Check if we already alerted for this raid
            ...
            # Timeout all recent joiners for raiding
            for join_time in recent_joins:
                # Find the member who joined at this time
    ...
    async def on_member_remove(self, member: discord.Member):
        ...
        # Timeout state is maintained by Discord automatically
        
        # Check if member had alert role and send ping notification
    ...
    async def handle_member_leave_notification(self, member: discord.Member):
        ...
        try:
            ...
            # Check if member had the alert role
            ...
            # Prepare fields for notification embed
            fields = [
                {"name": "User", "value": f"{member.mention} ({member.id})", "inline": True},
            ...
            if roblox_info:
                fields.extend([
                    {"name": "Roblox Username", "value": roblox_info.get("username", "Unknown"), "inline": True},
                    {"name": "Roblox Display Name", "value": roblox_info.get("displayName", "Unknown"), "inline": True},
                    {"name": "Roblox ID", "value": roblox_info.get("id", "Unknown"), "inline": True}
                ])
    ...
    async def get_roblox_info(self, user_id: int) -> dict:
        ...
        try:
            async with aiohttp.ClientSession() as session:
                # Get Roblox user ID from Bloxlink
                        ...
                        if roblox_id:
                            ...
                            async with session.get(roblox_url) as roblox_response:
                                if roblox_response.status == 200:
                                    ...
                                    avatar_url = f"https://thumbnails.roblox.com/v1/users/avatar-headshot?userIds={roblox_id}&size=150x150&format=Png&isCircular=false"
    ...
    async def send_ping_notification(self, guild: discord.Guild, ping_role: discord.Role, embed: EmbedDesign):
    ...
    async def check_invite_command(self, interaction: discord.Interaction, invite_url: str):
        """Check if a Discord invite is NSFW or inappropriate."""
        if not await StaffUtils.has_developer_permission_cross_guild(self.bot, interaction.user, "developer"):
        ...
        discord_invite_pattern = re.compile(r'discord(?:app)?\.com/invite/[a-zA-Z0-9-]+|discord\.gg/[a-zA-Z0-9-]+')
        if not discord_invite_pattern.match(invite_url):
            await interaction.followup.send("Invalid Discord invite URL format.", ephemeral=True)
            return
    ...
    # Untimeout commands available via moderation module
        ...
        try:
            ...
            for invite_code in invite_matches:
                try:
                    ...
                    # Check if the target guild is banned
                    ...
                    if server_ban:
                        ...
                        try:
                            ...
                            logger.warning(f"No permission to delete message with banned server invite in {message.guild.name}")
                        ...
                        break  # Stop checking after finding first banned server
    ...
    async def on_automod_action_execution(self, execution: discord.AutoModActionExecution):
        ...
        try:
            # Only handle blocked messages (not other actions like timeouts)
            if execution.action.type != discord.AutoModActionType.block_message:
                return
            
            # Skip if no content or from bot
            if not execution.content or execution.member.bot:
                return
                
            # Check if channel is ignored
            ignore_check = await self.bot.db.find_ignore(execution.guild.id, channel_id=execution.channel.id)
            if ignore_check:
                return
            ...
            mock_message = type('MockMessage', (), {
                ...
                'id': None,  # AutoMod blocked messages don't have message IDs
                'created_at': datetime.now(timezone.utc),
                'mentions': [],  # We'll parse this from content if needed
                'role_mentions': [],
                'bot': False
            })()
            
            logger.info(f"AutoMod blocked message detected: '{execution.content}' from {execution.member.display_name}")
            
            # Run suspicious activity detection on blocked content
            detector = get_suspicious_activity_detector(self.bot)
            await detector.check_message_patterns(mock_message)
            
            # Run AI moderation scanning on blocked content
            await self.scan_message_with_ai(mock_message)
            
            # Check for banned server invites in blocked content
            await self.check_banned_server_invites(mock_message)
            
            # NSFW detection on blocked content
            ...
            if is_nsfw:
                ...
                # Log the NSFW detection for blocked message
                await self.log_nsfw_detection(mock_message, execution.content, malicious_urls)
                
                # Log security event for AutoMod blocked NSFW content
                await self.security_logger.log_nsfw_detection(
                    user_id=execution.member.id,
                    guild_id=execution.guild.id,
                    channel_id=execution.channel.id,
                    message_id=None,  # No message ID for blocked messages
                    urls=malicious_urls,
                    content_length=len(execution.content)
                )
                
                # Additional security logging for AutoMod bypass attempt
