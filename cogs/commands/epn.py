...
class EPNCommands(commands.Cog):
    ...
    async def _safe_dm_user(self, user: Union[discord.User, discord.Member], embed: discord.Embed):
        """Send a DM without crashing the command if DMs are closed."""
        try:
    ...
    async def send_staff_log(self, guild: discord.Guild, embed: discord.Embed):
        ...
        try:
            ...
            if not log_config:
                logger.warning(f"No log config found for guild {guild.name} ({guild.id})")
                return

            channel = guild.get_channel(log_config["channel_id"])
            if not channel or not isinstance(channel, discord.TextChannel):
                logger.warning(
                    f"Configured log channel missing or invalid for guild {guild.name} ({guild.id})"
                )
                return
    ...
    async def send_cross_guild_log(
        ...
        """Send cross-guild action log to the configured channel for a specific guild."""
        try:
            ...
            logger.error(f"Error sending cross-guild log in {guild.name} ({guild.id}): {e}")
